...
Do not proceed. A vague brief creates vague tasks.
```
...
1. `vivia_workspace action='projects'`. Note the project identifier and pass it on every subsequent call (no server-side session state).
   - **Project-confirmation gate.** If `projects` returns multiple candidates
     whose titles or descriptions overlap the user's request, ASK before you
     proceed. Do not choose the closest match silently. Show the candidates and
     the user's stated intent: "I see `<A>` and `<B>` that could match. Which one
     are we decomposing?" The wrong project creates a graph that is hard to maintain
     and hard to undo.
...
- **Data model / domain entities**: entities and relationships. For non-CRUD projects, examples include physical models (simulation), tensors and pipelines (ML), event types (analytics), agent and tool surfaces (agentic), and HAL primitives (firmware).
...
If your working directory is sandboxed or write-restricted (CI runs, plugin test
rigs, or agents in a worker subfolder), `.vivia/` may not be writable. Use a
writable directory in the sandbox and record its path in the `## Decomposition Plan`
block from Step A. Resume mode can then find it. If no local writes are
possible, skip Step B and use the project-description plan from Step A. Record
this limitation in your transcript. Progress will not survive compaction on
another session.
...
**Do not skip either step.** Step A keeps the plan recoverable across machines.
Step B keeps progress and in-flight notes recoverable after compaction. Together
they prevent a new session from creating BAT-1..12 on top of existing BAT-1..12.
...
1. `vivia_workspace action='update' status='decomposing'`. Flip the phase before
   the first write. A project already in `decomposing` has an interrupted run.
   Resume from the working file. Do not restart.
...
Catching drift at task 15 is a 30-second fix. The same drift at task 50 means
rewriting 35 tasks. Do not skip the checkpoint.
...
**Offer cleanup. Do not auto-clean.** The user may want to keep the plan as an
audit trail or the working file for review. Ask before you remove either item.
...
If the user approves, delete `.vivia/decompose-<projectIdentifier>.md`. Remove
...
- Your sandbox cannot delete files (write-restricted, non-POSIX shell with no equivalent, or another limit). Surface the limitation and ask the user to clean up the working file manually. Step 1 (description refresh) still works because it uses an MCP tool call.
...
- NEVER write text into Vivia in a chatbot style. No em dash punctuation, no
  marketing words ("comprehensive", "robust", "leverage"), and no AI
  throat-clearing. Artifacts §6.
