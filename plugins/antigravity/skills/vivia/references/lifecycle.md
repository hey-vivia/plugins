...
- `archived`: read-only on the task surface. `vivia_create`, `vivia_edit`, and `vivia_link` fail; reads still work. A human or manage workflow archives the project. Unarchive via `vivia_workspace action='update' status='active'`.

The server emits `_hints` when a write does not match the phase, for example when a task moves to `in_progress` while the project is `decomposing`. Act on these hints.
...
- **Constraint:** Do not let work span sessions. If work pauses, leave a note in
  the task or move it back to `planned`.
...
- **What it means.** The work shipped and was approved. The PR is merged (or
  otherwise accepted), and the HOTL operator flipped the task from `in_review`.
  The task carries the full record: `executionRecord` (3-5 sentences on what
  was built), `decisions` (one line per choice), `files` (every path touched),
  and `acceptanceCriteria` with each item evaluated (`checked: true` or `false`).
...
For pure spec-review, docs, decision-only, or Vivia-only refinement tasks that
touch no repo files, use `set files` with `value=[]`. If you omit the op, the prior
value stays in place and the server's "missing files" hint remains. The empty
array answers "what changed in the repo?" with no paths.
...
4. Create, update, or remove edges when required via `vivia_link` (keyed by source+target+type).
