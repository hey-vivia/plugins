...
Decisions come from the refinement, planning, or implementation conversation.
When the user and the agent, or two agents, settle on a choice, record it as a
decision. Record it without waiting for a request. If you are unsure whether a
choice is a decision, ask the user for confirmation.
