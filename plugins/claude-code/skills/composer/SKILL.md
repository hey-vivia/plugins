...
Composer coordinates these steps. The `vivia` skill (`skills/vivia/SKILL.md`)
contains task selection, refinement, the Completion Protocol, and propagation.
Composer reuses those flows instead of duplicating them.
...
1. **Resolve the project.** Run `vivia_workspace action='projects'` and note the
   identifier. Pass it, or a taskRef, on every call. The server does not store a
   selection. In single-task mode, also run `vivia_search query='<taskRef>'` to
   confirm the task and its status.
...
5. **Init the run log.** Run `mkdir -p .vivia` and guard the gitignore
   (`grep -qxF '.vivia/' .gitignore 2>/dev/null || printf '\n.vivia/\n' >> .gitignore`). If `.vivia/composer-<projectIdentifier>.md` ends with
   `RUN_END`, archive it to
   `.vivia/archive/composer-<projectIdentifier>-<date>.md` and start a new log.
   If the file has no `RUN_END`, use *Recovering after compaction* first. If its
   `RUN_START mode=` differs from this invocation, append
   `RUN_END reason=superseded-by-<mode>`, archive it, and start a new log. Then
   append `RUN_START mode=<...> mergePolicy=<...> project=<identifier>`.
...
2. **Gather pick facts and launch.** Build workflow `args` from the pick and
   bootstrap. Include `taskRef`, `taskId`, `projectId`, `categories`,
   `tagVocabulary`, `pickEstimate`, `pickPriority`, `workType`, `tags`, `mode`,
   and `plannableOnly`. Refs are first-class in tool calls (conventions §4).
   Write `PICK`, then `WORKFLOW task=<ref> runId=<id>` to the run log. Launch the
   workflow and wait for the result.
...
- **Low confidence or external input** (confidence < 0.6,
  `external-input-required`, or any plan-phase open question): show the open
  questions and wait for answers. Relaunch a research gate **fresh** with
  `gateAnswers`. Relaunch a plan gate with `resumeFrom='plan'`,
  `priorBrief=result.brief`, and `gateAnswers`. This keeps the prior research.
...
To merge, run `gh pr merge <url> --squash --delete-branch`. Squash is the
default. Follow the repository's configured method when it differs. After a
clean merge, write the task `done`. This is the **one** status transition that
the orchestrator writes. The run-start merge policy authorizes it.
...
Per-phase events and fix rotations live in the workflow's journal, not the run
log. The `WORKFLOW runId` line links the two. If `.vivia/` is not writable, use
another writable directory and name it in the first report. If no local write is
possible, run without the log and say so. The run loses crash recovery, not
correctness.
...
The workflow builds each phase dispatch from the `args` you pass. Agents inherit
nothing else. Keep `args` to the pick facts in *Step details*. Never pass the
orchestrator transcript, prior-iteration summaries, full meta payloads, or Vivia
reference text. Agents load their own rule extracts and fetch task context from
Vivia. Oversized dispatches reduce quality.

## Failure handling

`BLOCKED`/null from the workflow is a failed attempt, with exceptions:

- A phase that reports BLOCKED because the task is already `done` or `cancelled`
  is not a failure. HOTL resolved it during the run. Run *Surface + merge +
  propagate* if needed. Consume no budget and continue.
- `BLOCKED — environmental: <error>` (gh auth, rate limits, network) is an
  environment problem. Show the error verbatim. Consume no budget. Resume the
  workflow through `resumeFrom` after the user confirms the fix.
...
The workflow dispatches the implementer and each fix rotation with
`isolation:'worktree'` (`compose-task.js`). Each task leaves a git worktree under
`.claude/worktrees/wf_<runId>-<n>` and a local branch. The harness removes a
worktree only when it is unchanged. The implementer commits, so a worktree can
remain after the task ends. The merge gate removes a merged task's worktree
through `gh pr merge --delete-branch`; that command removes only the remote
branch. The local cleanup happens here. At every stop, after `RUN_END` and before
the archive offer, show the remaining worktrees and clean them up according to
the merge policy. At run end, the remaining worktrees are PR-backed worktrees,
orphan worktrees, or all worktrees under `never`. Composer does not change the
filesystem without instruction.

1. **Enumerate.** Run `git worktree list --porcelain`. Keep only paths under
   `.claude/worktrees/wf_*`. The `wf_<runId>` prefix identifies worktrees that
   the workflow created. Exclude the primary checkout and user-created
   worktrees. Capture each path and its `branch refs/heads/<name>`.
2. **Classify against open PRs.** Run `gh pr list --state open --json number,headRefName,url`. A worktree whose branch equals an open PR's `headRefName` is **PR-backed** and may be at `in_review` awaiting HOTL. Every other worktree is **safe**. Its branch backs no open PR, or it is an orphan `worktree-wf_*` branch. If `gh` is unavailable or errors, treat every worktree as PR-backed and inform only.
3. **Report.** Per bucket, list each worktree path, its branch, and the PR (PR-backed); say so explicitly when none remain. Always print the exact commands:
   ...
   - dangling local branch: `git branch -D <branch>`. `-D`, not `-d`, is
     intentional. Orphan `worktree-wf_*` branches never merge upstream, so `-d`
     refuses them. For a PR-backed branch, this drops only the *local* ref. The
     PR, remote branch, and `git fetch` recovery remain.
   - stale entries whose directory is already gone: `git worktree prune`.
4. **Clean up (per merge policy).**
   - **`auto-on-approve`**: auto-remove the safe bucket. Run the `git worktree remove` and `git branch -D` pair for each worktree, then run `git worktree prune`. Do not prompt. The run-start delegation that authorized auto-merge covers this cleanup. Show PR-backed worktrees (step 3), but do not remove them automatically.
   - **`never` / `ask-each`**: ask once with the AskUserQuestion tool whether to
     remove safe worktrees, remove all worktrees including PR-backed ones, or
     leave all worktrees. Remove only the selected bucket.
   - **Headless** (AskUserQuestion unavailable): inform only; remove nothing.

## Recovering after compaction

Read `.vivia/composer-<projectIdentifier>.md` first. The last `PICK` or
`WORKFLOW` without a matching `TASK_END` identifies the in-flight task. **Use
Vivia for status.** Re-read the task row instead of trusting the log. **Use the
log for history.** It records the merge policy (`RUN_START mergePolicy=`), gate
answers, verdict history, and workflow `runId`.

To resume the in-flight task:

- A `WORKFLOW runId=<id>` line with no `VERDICT` or `TASK_END` after it means the
  workflow may still be journaled. Resume it with
  `Workflow({ scriptPath, resumeFromRunId: '<id>' })`. Completed phases return
  from cache. Only the unfinished phase runs again. Stop the prior run first if
  it is still active.
...
| Temptation | Reality |
| --- | --- |
| Write `status` "so no other agent grabs the task" | Every transition belongs to a phase agent: planner `draft→planned`; implementer `planned→in_progress→in_review` plus fix rotations. The orchestrator writes propagation edges. It writes `done` only when the merge gate merges the PR under an authorizing merge policy. |
...
- `skills/vivia/SKILL.md`: canonical flows that composer reuses: selection, refinement, planning, implementation, and propagation.
