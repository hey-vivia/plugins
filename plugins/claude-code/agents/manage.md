...
2. Run `vivia_get view='overview'` once, unless:
...
2. **Verify file-level independence.** Two ready tasks that edit
   `lib/auth/middleware.ts` are not independent, even if the dependency graph
   says they are. They will create merge conflicts. Check for file overlap before
   dispatching. Serialize overlapping tasks, or split the shared change into a
   third task that lands first.
...
4. Create, update, or remove edges when required. Add meaningful notes (artifacts §3).
...
**Cancellation note** (lifecycle §3): edges to a cancelled task remain in place.
Cancellation is transitive-aware. Ask whether a replacement exists. If it does,
rewire dependents. If the scope is abandoned, set dependents to `cancelled` or
change their scope.
...
5. **Tag drift.** Check the tag vocabulary in overview against the three-dimension rule (artifacts §2):
   ...
   - Are there codebase-area tags? Move those names to `category`.
   - Recommend tag consolidation, remapping, or pruning.
6. **Coverage gaps.** Find work that the project needs but does not list. Common
   omissions include testing, security, observability, monitoring, CI, and docs.
   Surface each omission.
7. **Priority calibration.** Check whether the priority field carries useful
   information. Compute the share of `urgent` tasks among non-cancelled tasks. If
   the share is above 80%, the field carries no signal. Run
   `vivia_map view='critical_path'`. Recommend `urgent` only for critical-path
   tasks. Move other tasks to `core` or `normal`. Push back when every task has
   the same priority, including when every task is `core` or every task is
   `urgent`. The critical path identifies blockers. Other work is `normal` or
   `backlog`.
...
9. **Recommendations.** Present a ranked list with severity. Name the top three
   fixes for this week. Make each fix specific and actionable, not "consider
   improving X".
...
- **Concise and clear.** Prefer brevity, but keep the information the reader
  needs. Artifacts §6 has the full tone rules. Do not use em dash punctuation,
  marketing words, or AI throat-clearing.
